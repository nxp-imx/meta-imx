...
EXTRA_OESCONS = "build=cross_compile os=linux toolchain_prefix=' ' extra_cxx_flags='-fPIC' ${PACKAGECONFIG_CONFARGS}"
EXTRA_OESCONS += "${@bb.utils.contains('TARGET_ARCH', 'aarch64', 'arch=arm64-v8a neon=1', '', d)}"

TARGET_CC_ARCH += "${LDFLAGS}"

# Workaround to fix default scons_do_compile which includes a non-existing prefix option
do_compile() {
