...
REMOVE_GL = "${@base_contains('DISTRO_FEATURES', 'wayland', \
                   base_contains('DISTRO_FEATURES', 'x11', 'gl', 'gl', d),'', d)}"
...
python __anonymous() {
    ...
    if "gles2" in packageconfig:
        if bb.utils.contains('DISTRO_FEATURES', 'x11', \
                 bb.utils.contains('DISTRO_FEATURES', 'wayland', False, True, d), False, d):
         flavors.append("x11-glesv2")
        elif bb.utils.contains('DISTRO_FEATURES', 'wayland', \
                 bb.utils.contains('DISTRO_FEATURES', 'x11', True, True, d), False, d):
