...
#
# MACHINE_MX8ARCH_FILTER list all packages associated with
# MACHINE_MX8ARCH and, when match, will set PACKAGE_ARCH as MACHINE_MX8ARCH
#
...
# MACHINE_SOCARCH_FILTER_soc = "imx-codec ...imx-vpu"
# MACHINE_MX8ARCH_FILTER = "virtual/libgl ...virtual/libgles1"
#
...
python __anonymous () {
    ...
    machine_mx8arch_filter = set((d.getVar("MACHINE_MX8ARCH_FILTER", True) or "").split())

    if machine_socarch_filter or machine_arch_filter or machine_mx8arch_filter:
        ...
        elif list(machine_mx8arch_filter & (provides | depends)):
            package_arch = d.getVar("MACHINE_MX8ARCH", True)
            if not package_arch:
                raise bb.parse.SkipPackage("You must set MACHINE_MX8ARCH as MACHINE_MX8ARCH_FILTER is set.")

            machine_mx8arch_suffix = d.getVar("MACHINE_MX8ARCH_SUFFIX", True)
            if not machine_mx8arch_suffix:
                raise bb.parse.SkipPackage("You must set MACHINE_MX8ARCH_SUFFIX as MACHINE_MX8ARCH_FILTER is set.")
    ...
    machine_mx8arch = (d.getVar("MACHINE_MX8ARCH", True) or "")
    if not machine_socarch in cur_package_archs:
        d.appendVar("PACKAGE_EXTRA_ARCHS", " %s" % machine_socarch)
    if not machine_mx8arch in cur_package_archs:
        d.appendVar("PACKAGE_EXTRA_ARCHS", " %s" % machine_mx8arch)
...
MACHINE_MX8ARCH = "${TUNE_PKGARCH}${MACHINE_MX8ARCH_SUFFIX}"
