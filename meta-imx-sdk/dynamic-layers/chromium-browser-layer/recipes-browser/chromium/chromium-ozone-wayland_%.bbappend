...
SRC_URI:append:imx-nxp-bsp = " \
    ...
    file://0007-Fix-build-fail-after-clang-llvm-upgrade.patch \
    file://0008-Fix-chromium-crash-when-run-webgl-2.0.0-cts.patch \
    file://0009-Fix-canvas-test-fail-for-webgl.patch \
    file://0010-LF-12406-1-Blacklist-disable-program_caching_for_tra.patch \
    file://0011-Disable-GL_ANGLE_instanced_arrays-extension.patch \
    file://0012-MGS-8560-Fixed-webgl-random-failures-with-angle-gles.patch \
    file://0013-MGS-8643-Fix-chromium-fail-to-run-with-Panthor-drive.patch \
...
VDA_PATCH_SET = " \
    ...
    file://0117-V4L2VideoDecoder-Fix-amphion-cannot-streamoff-after-.patch \
    file://0118-V4L2VideoDecoder-Set-OUTPUT-format-with-parsed-resol.patch \
    file://0119-V4L2VideoDecoder-Add-back-HEVC-code-for-stateful-vid.patch \
    file://0120-V4L2VideoDecoder-Disable-decoder-prioritization-acco.patch \
    file://0121-V4L2VideoDecoder-Fix-cannot-select-V4L2VD.patch \
"
...
do_copy_clang_library () {
    ...
    if [ "${TARGET_ARCH}" != "${BUILD_ARCH}" ]; then
        ...
        rm -rf "latest/lib/${BUILD_ARCH}-unknown-linux-gnu"
        cp -ar latest/lib/linux "latest/lib/${BUILD_ARCH}-unknown-linux-gnu"
        cd "latest/lib/${BUILD_ARCH}-unknown-linux-gnu"
